...
class ImportMultipleFilesUseCase @Inject constructor(
    ...
    suspend operator fun invoke(uriStrings: List<String>): Result<List<Long>> = withContext(Dispatchers.IO) {
        try {
            val successfulIds: MutableList<Long> = mutableListOf()
            for (uri: String in uriStrings) {
                importDocumentUseCase(uri)
                    .onSuccess { id: Long -> successfulIds.add(id) }
                    .onFailure { exception: Throwable -> throw exception }
            }
